...
static void window_load(Window *window) {
  ...
  // The explicitely set callback assures that SELECT button can be used. 
  ...
  text_layer_instructions = text_layer_create(GRect(0, 0, bounds.size.w, 2000));     
  text_layer_set_text(text_layer_instructions, "Currently there are no notifications received from the iPhone. Please wait ...");
  ...
  scroll_layer_set_content_size(scroll_layer, GSize(bounds.size.w, max_size_instructions.h + vert_scroll_text_padding));
